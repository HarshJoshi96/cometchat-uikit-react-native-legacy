...
import { ConversationTypeConstants, GroupTypeConstants, MessageCategoryConstants, MessageStatusConstants, PASSWORD_GROUP_COLOR, PRIVATE_GROUP_COLOR, ReceiverTypeConstants } from "../shared/constants/UIKitConstants";
...
export const CometChatConversations = (props: ConversationInterface) => {
    ...
    const ErrorView = () => {
        return (
            <View style={Style.listContainer}>
                <Text style={{ ..._style?.errorTextFont, color: _style?.errorTextColor } as TextStyle}>
    ...
    const EmptyView = () => {
        if (EmptyStateView)
            ...
            return (
                <View style={Style.listContainer}>
                    <Text style={{ ..._style.emptyTextFont, color: _style.emptyTextColor } as TextStyle}>
        ...
        if (conversation) {
            ...
            if (isTyping && !newConversation?.['lastMessage']?.["typing"]) {
                newConversation['lastMessage']["typing"] = args[0]?.receiverType === 'group' ?
                    `${args[0].sender.name.trim()} ${localize("IS_TYPING")}` :
                    localize("IS_TYPING");
            } else {
  ...
  const shouldUpdateLastMessageAndUnreadCount = (
    ...
    const updateLastMessage = (newMessage: CometChat.BaseMessage) => {
        ...
        CometChat.CometChatHelper.getConversationFromMessage(newMessage)
            .then((conversation: any) => {
                ...
                const oldConversation: CometChat.Conversation | any = conversationListRef.current?.getListItem(conversation['conversationId']);
                ...
                if ((newMessage as any)['sender']['uid'] != loggedInUser.current?.['uid'])
     ...
     */
    const markMessageAsDelivered = (message: object) => {
    ...
    const groupHandler = (message: any, otherDetails: {action?: string, actionOn?: CometChat.User, actionBy?: CometChat.User, group?: CometChat.Group} = {}) => {
        ...
        let {action, actionOn, actionBy, group} = otherDetails;
          if (conversation) {
            if ((action && ["kicked", "banned", "left"].includes(action)) && (actionOn?.getUid() == loggedInUser.current?.getUid())){
    ...
    const conversationClicked = (conversation: any) => {
        if (onItemPress) {
        ...
        let index = selectedConversation.findIndex((tmpConver: any) => tmpConver.conversationWith.conversationId == conversation.conversationWith.conversationId);
        if (index < 0) {
    ...
    const conversationLongPressed = (conversation: any) => {
        if (onItemLongPress) {
    ...
    const removeConversation = (id: string | number) => {
        let conversation: any = conversationListRef.current?.getListItem(id);
    ...
    const getMessagePreview = (conversations: CometChat.Conversation, uid: any) => {
    ...
    function getFormattedText(message: CometChat.BaseMessage, subtitle: string) {
        ...
        if (!disableMentions && message.getMentionedUsers().length) {
            ...
            CometChatUIKit.loggedInUser && mentionsFormatter.setLoggedInUser(CometChatUIKit.loggedInUser);
    ...
    const getDefaultOptions = (conversation: any) => {
        ...
        _defaultOptions[0].onPress = (id: any) => {
            setConfirmDelete(id);
        };
    ...
    const updateConversationLastMessage = (message: any) => {
        ...
        try {
            let conversation: CometChat.Conversation | any = conversationListRef.current?.getListItem(message.conversationId)
    ...
    React.useEffect(() => {
        ...
        CometChatUIEventHandler.addGroupListener(
            groupListenerId,
            {
                ...
                ccOwnershipChanged: ({ message }: any) => {
                    if(!(shouldUpdateLastMessageAndUnreadCount(message))) {
                        return;
                    }
                    CometChat.CometChatHelper.getConversationFromMessage(message)
